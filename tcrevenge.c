...
const char *firmware_version = "7.3.165.1_v011\n";	// this looks that is not actually used
const char* hardware_version = "PT632_2-CT-GD-V3.5.3\r\n";
const char* board_version = "HardWare Ver. 0001\r\n";
...
const unsigned int hardware_version_offset = 0x80;
const unsigned int board_version_offset = 0xA0;
...
int main(int argc, const char *argv[]) {
  ...
  if (mode == CREATE) {
   ...
   // sum = calc_crc32(sum, paddingfile, 0);
    ...
    set_int(header,0x7c,0x80002000);
    printf("Hardware version at 0x%02X: %s\n", hardware_version_offset, strip_newline(hardware_version));
    set_string(header, hardware_version_offset, hardware_version);    
    printf("Board version at 0x%02X: %s\n", board_version_offset, strip_newline(board_version));
    set_string(header, board_version_offset, board_version);    
